/**
 * Minimal climate data population - just enough to demo the platform
 * Populates data for 50 counties over 30 days
 */
...
function calculateGDD(tempMax: number, tempMin: number, baseTemp: number = 10): number {
...
function getBaseTemperature(state: string): number {
  const tempMap: Record<string, number> = {
    'FL': 25, 'TX': 22, 'CA': 18, 'AZ': 24,
    'MN': 8, 'ND': 6, 'SD': 8, 'WI': 10,
    'IA': 12, 'IL': 13, 'IN': 12, 'OH': 12,
  };
...
function getDroughtRisk(state: string): number {
  const riskMap: Record<string, number> = {
    'CA': 0.8, 'AZ': 0.9, 'NV': 0.85, 'NM': 0.8,
    'TX': 0.6, 'OK': 0.6, 'KS': 0.5,
  };
...
async function populateMinimalData() {
  ...
  const { data: counties, error: countiesError } = await supabase
    .from('counties')
    .select('fips, state')
    .limit(50); // Only 50 counties for demo
  ...
  for (const county of counties) {
    ...
    for (let d = new Date(startDate); d <= endDate; d.setDate(d.getDate() + 1)) {
      ...
      const tempMax = baseTemp + seasonalFactor * 10 + (Math.random() - 0.5) * 5;
      const tempMin = tempMax - 8 - Math.random() * 4;
      const tempAvg = (tempMax + tempMin) / 2;
      
      const precipitation = Math.max(0, basePrecip * (1 + seasonalFactor * 0.3) * Math.random() * 2);
      const soilMoisture = Math.max(10, Math.min(100, 50 + seasonalFactor * 20 + (Math.random() - 0.5) * 30));
      ...
      if (droughtRisk > 0.7 && soilMoisture < 20) droughtIndex = Math.min(4, droughtIndex + 1);
  ...
  for (let i = 0; i < climateData.length; i += batchSize) {
    ...
    if (error) {
      ...
      console.log(`✓ Inserted ${Math.min(i + batchSize, climateData.length)}/${climateData.length} climate records`);
    }
  }
  ...
  for (let i = 0; i < gddData.length; i += batchSize) {
    ...
    if (error) {
      ...
      console.log(`✓ Inserted ${Math.min(i + batchSize, gddData.length)}/${gddData.length} GDD records`);
    }
  }
